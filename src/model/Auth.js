...
module.exports = {
                ...
                bcrypt.compare(password, hashedPassword, (error, result) => {
                  ...
                  if (result) {
                    const tokenJWT = jwt.sign(
                      { email, id, name, roleId, device_token },
  ...
  checkDevice: (email, token) => {
    return new Promise((resolve, reject) => {
      console.log(email, token);
      console.log("from prepare model");
      db.query(
        `SELECT device_token FROM user WHERE email='${email}'`,
        (err, res) => {
          console.log(res.device_token);
          console.log("from model cek device");
          if (!err) {
            if (res[0].device_token == token) {
              resolve(res);
            } else if (res[0].device_token == "nothing") {
              db.query(
                `UPDATE user SET device_token='${token}' WHERE email='${email}'`,
                (err, res) => {
                  if (!err) {
                    resolve(res);
                  } else {
              ...
              return reject(" Devices already login in another device. ");
            }
          } else {
            return reject(err);
          }
