...
module.exports = {
  ...
  transactionHistory: async function (req, res) {
    try {
      ...
      const [
        ...
        transactionModel.transactionDetail(token, limit, dateStart, until),
      ]);
      const result = {
        inWeek: inWeek,
        inMonth: inMonth,
  ...
  createTransfer: async function (req, res) {
    try {
      ...
      // console.log("req.body from transaction");
      const token = req.token;
